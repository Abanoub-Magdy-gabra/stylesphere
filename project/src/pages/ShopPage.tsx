...
// Mock function to filter products based on search and filters
const filterProducts = (products: typeof featuredProducts, searchQuery: string, filters: any) => {
  return products.filter((product: any) => {
    ...
    // Filter by sizes (if product has sizes)
    ...
    // Filter by colors (if product has colors)
...
const ShopPage = () => {
  ...
  useEffect(() => {
    const filtered = filterProducts(featuredProducts, searchQuery, filters);
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
          <h1 className="text-3xl font-serif font-bold">Shop All</h1>
          ...
          {(searchQuery || filters.category.length > 0 || filters.sizes.length > 0 || filters.colors.length > 0 || filters.priceRange[1] < 1000) && (
            <div className="mb-6 flex flex-wrap items-center gap-2">
              ...
              {filters.sizes.map((size: string) => (
                <span key={size} className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-gray-100 text-gray-800">
                  Size: {size}
                  <button
                    type="button"
                    onClick={() => setFilters((prev: any) => ({
              ...
              {filters.colors.map((color: string) => (
                <span key={color} className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-gray-100 text-gray-800">
                  Color: {color}
                  <button
                    type="button"
                    onClick={() => setFilters((prev: any) => ({
